...
# Validate the Google Cloud CLI installation and version
function validate_gcloud(){
    echo -e "\n\e[34m»»» ✅ \e[96mValidating gcloud Installation..."
    if [ ! "$(command -v gcloud)" ]; then
        ...
        if [ "$(compare_version "$GCLOUD_CURRENT_VERSION" "$GCLOUD_CLI_VERSION")" -eq 2 ]; then
            echo_wrong_version "gcloud CLI" "at least" "$GCLOUD_CLI_VERSION" "https://cloud.google.com/sdk/docs/install" "$GCLOUD_CURRENT_VERSION"
            ERRORS+=$'  gcloud version is incompatible.\n'
        fi
        echo -e "\n\e[34m\t👍 \e[96mGCLOUD Version found: $GCLOUD_CURRENT_VERSION"
    fi
}

# Validate the configuration of the gcloud
function validate_gcloud_configuration(){
    echo -e "\n\e[34m»»» ✅ \e[96mValidating gcloud Configuration..."
    gcloud config set project "${PROJECT_ID}"
    export PROJECT_ID="$(gcloud config get-value project 2> /dev/null)"
    export SERVICE_PRINCIPAL_ID="$(gcloud config get-value account 2> /dev/null)"
    
    if [ "$(echo "$SERVICE_PRINCIPAL_ID" | grep -c unset)" -eq 1 ]; then
        echo -e "\n\e[31m\t⚠️ You are not logged in to Google Cloud! You must configure an End User Credential.!"
        echo -e "\n\e[31m\t⚠️ try using $BOOTSTRAP_DIR/scripts/gcloud-login.sh!"
        echo -e "\e[31m\t😥 Visit https://cloud.google.com/sdk/gcloud/reference/auth/login and follow the instructions\e[0m"
...
function main(){
    ...
    echo ""
    echo ""
    echo -e "\n\e[34m🔨 \e[96mGoogle Cloud details from logged on user \e[0m"
    echo -e "\e[34m\t• \e[96mProject:       \e[33m$PROJECT_ID\e[0m"
    echo -e "\e[34m\t• \e[96mmUser:        \e[33m$SERVICE_PRINCIPAL_ID\e[0m\n"
...
echo $PROJECT_ID $SERVICE_PRINCIPAL_ID
